...
exports.getAdminAnalytics = async (req, res, next) => {
  try {
    // ⚡ Bolt: Batch independent analytics queries concurrently to prevent cumulative I/O blocking
    const [totalUsers, activeUsers, moviesUploaded, watchedAgg, searchAgg, revenueAgg] = await Promise.all([
      User.countDocuments(),
      ...
      // Optimize monthly revenue calculation using MongoDB aggregation instead of fetching all users into memory
      User.aggregate([
