...
## 2024-05-18 - Cumulative I/O Blocking in Mongoose
**Learning:** Sequential execution of independent database queries (`await Model.find()`, `await Model.aggregate()`, etc.) causes cumulative network and I/O blocking, needlessly increasing the total response time for endpoints like dashboards and analytics.
**Action:** Always wrap independent Mongoose queries and aggregations in `Promise.all()` to execute them concurrently, resolving multiple I/O bounds in the time of the slowest single query.
