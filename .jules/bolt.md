...
## 2024-06-13 - Promise.all for Mongoose I/O Batching
**Learning:** Sequential `await` calls on independent Mongoose database queries (e.g., in analytics or dashboards where multiple distinct pieces of data are needed) cause cumulative I/O blocking, unnecessarily multiplying the overall endpoint response time by the number of queries. Mongoose handles concurrent queries gracefully.
**Action:** Always batch independent database operations (like `countDocuments`, `find`, or `aggregate`) using `Promise.all()` to execute them concurrently, reducing total query time to approximately the duration of the slowest single query.
