...
## 2024-07-05 - Sequential Database Lookups Block Main Thread
**Learning:** Executing independent database queries (like aggregations and counts for analytics dashboards) sequentially using separate `await` statements accumulates the total wait time (O(M+N+O...)).
**Action:** When aggregating data from multiple collections for a single API response, group the independent queries using `Promise.all()` to execute them concurrently, reducing total wait time to O(max(M, N, O...)).
