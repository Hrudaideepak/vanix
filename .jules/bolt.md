...
## 2026-07-26 - Concurrent Mongoose Queries
**Learning:** Sequential, independent database read queries in controllers (like analytics dashboards) cause cumulative I/O wait times and slow down endpoint responses.
**Action:** Always batch independent Mongoose database queries (e.g., `countDocuments`, `find`, `aggregate`) using `Promise.all()` to execute them concurrently, preventing cumulative I/O blocking.
