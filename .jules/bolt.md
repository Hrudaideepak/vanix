...
## 2024-06-12 - Concurrent Database Queries Pattern
**Learning:** Sequential execution of multiple independent database aggregations and counts in dashboard endpoints (e.g., `analytics.controller.js`) causes unnecessary cumulative I/O blocking and increased latency.
**Action:** Always batch independent Mongoose `countDocuments`, `find`, and `aggregate` calls using `Promise.all()` to execute them concurrently, reducing total execution time to the duration of the longest single query.
