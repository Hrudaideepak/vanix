...
## 2024-06-25 - Batching Independent Database Queries
**Learning:** Sequential execution of independent Mongoose queries (e.g., `countDocuments`, `aggregate`) in controllers can cause cumulative network and database latency, significantly slowing down endpoint response times, especially in analytical dashboards.
**Action:** Use `Promise.all()` to batch independent Mongoose database queries to execute them concurrently and prevent I/O blocking bottlenecks.
