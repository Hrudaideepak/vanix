...
## 2026-06-30 - Concurrent DB Queries
**Learning:** Running independent database queries sequentially causes cumulative I/O blocking and unnecessarily slows down API endpoints, particularly those aggregating data like analytics dashboards.
**Action:** Batch independent Mongoose database queries (e.g., `countDocuments`, `find`, `aggregate`) using `Promise.all()` to execute them concurrently, reducing total execution time to the duration of the slowest single query.
